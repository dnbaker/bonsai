...
inline void bitmap_filler_helper(void *data_, long index, int tid) {
...
inline std::vector<tax_t> binary_new2old_map(const char *fn) {
    gzFile fp(gzopen(fn, "wb"));
...
inline std::vector<tax_t> build_new2old_map(const char *path, size_t bufsz) {
