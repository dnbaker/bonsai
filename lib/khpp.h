...
template<typename khkey_t, typename khval_t,
         ...
         bool is_map=true, size_t BUCKET_OFFSET=6>
struct khpp_t {
    ...
    index_type iput(const khkey_t &key, int *ret, Args &&... args)
    {
        ...
        std::cerr << "Acquiring lock in iput\n";
        std::shared_lock<decltype(m)> lock(m);
        std::cerr << "Acquired lock in iput\n";
        tthread::fast_mutex &local_lock(locks[x >> BUCKET_OFFSET]);
        std::cerr << "Size of locks: " << locks.size() << '\n';
        assert((x >> BUCKET_OFFSET) <= locks.size());
        std::cerr << "Acquiring local lock\n";
        local_lock.lock();
        std::cerr << "Acquired local lock\n";
#if 0
        ...
        if (__ac_isempty(flags, x)) { /* not present at all */
            keys[x] = key;
            flags[x>>4] &= ~(3ull<<((x&0xfU)<<1));
            assert(!__ac_iseither(flags, x));
            ++size;
            ++n_occupied;
            new (vals + x) khval_t(args...);
            *ret = 1;
        ...
        assert(exist(x));
        auto ix(iget(key));
        if(ix == n_buckets) throw std::runtime_error(std::string("ZOMG IT IZ N BUCKETS ") + std::to_string(ix));
        //for(khiter_t kil(0); kil != n_buckets; ++kil) if(exist(kil)) assert(iget(kil) != n_buckets);
        local_lock.unlock();
        return x;
    }
    ...
    { /* This function uses 0.25*n_buckets bytes of working space instead of [sizeof(key_t+val_t)+.25]*n_buckets. */
        std::cerr << "Acquiring resize lock\n";
        std::cerr << "Acquired resize lock\n";
        ...
        if (j) { /* rehashing is needed */
            ...
            if((n_buckets >> BUCKET_OFFSET) > locks.size()) locks.resize(n_buckets >> BUCKET_OFFSET);
        }
        return 0;
    }
    template<typename T>
