...
def parse_assembly(fn, fnidmap):
    ...
    for line in open(fn, encoding='utf8'):
        ...
        assert len(s[0]) > 10
        duo1, duo2 = s[0][10:12], s[0][8:10]
        assert is_integer(duo1)
        assert is_integer(duo2)
        ...
        fnidmap[(duo1, duo2, fn)] = int(s[5])
        index = len(s) - 1
        while "ftp" not in s[index] and index > 0:
            index = index - 1
        if index:
            to_fetch.append((duo1, duo2, s[index] + "/" + fn))
        else:
...
def main():
    ...
    if argv[1:] and argv[1] == "nodes":
        if not os.path.isfile("%s/nodes.dmp" % ref):
            cc("curl -s --create-dirs {tax_path} -o {ref}/"
    ...
    for clade in to_dl:
        ...
        if not os.path.isfile("%s/%s/as.%s.txt" % (ref, clade, clade)):
            cstr = ("curl -s --create-dirs %s/assembly_summary.txt "
        ...
        with multiprocessing.Pool(args.threads) as spoool:
            spoool.map(check_path_lazy if args.lazy else check_path,
                       ("/".join([ref, clade, s[0], s[1], s[2].split("/")[-1]]) for
                        s in to_dl[clade]))
        print("Checked existing paths for download of clade " + str(clade), file=sys.stderr, flush=True)
        cstrs = [("curl -s --create-dirs %s -o %s/%s/%s/%s/%s" %
                 (s[2], ref, clade, s[0], s[1], s[2].split("/")[-1])) for
                 s in to_dl[clade] if not os.path.isfile(
                     "%s/%s/%s/%s/%s" % (ref, clade, s[0], s[1], s[2].split("/")[-1]))]
        # If nodes.dmp hasn't been downloaded, grab it.
        if not os.path.isfile("%s/nodes.dmp" % ref):
            cstrs.append("curl -s {tax_path} --create-dirs -o {ref}/"
                         "taxdump.tgz && tar -zxvf {ref}/taxdump.tgz"
                         " && mv nodes.dmp {ref}/nodes.dmp".format(**locals()))
        ...
        for i, tup in enumerate(list(cladeidmap.keys())):
            duo1, duo2, fn = tup
            try:
                #print(ref, clade, fn)
                cladeidmap[xfirstline("/".join(
                    [ref, clade, duo1, duo2, fn]
                )).decode().split()[0][1:]] = cladeidmap[tup]
                del cladeidmap[tup]
